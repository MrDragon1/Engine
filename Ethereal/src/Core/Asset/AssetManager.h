...
class AssetManager {
   public:
    ...
    static const std::string GetDisplayTypeName(AssetHandle handle) { return Utils::AssetTypeToString(GetMetadata(handle).Type); }

    static const std::string GetAssetName(AssetHandle handle) { return AssetManager::GetMetadata(handle).FilePath.stem().string(); }
    ...
    static TextureHandle GetAssetIconFromExtension(const std::string& extension);
