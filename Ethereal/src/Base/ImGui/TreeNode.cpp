...
namespace ImGui
{
    bool TreeNodeWithIcon(Ethereal::Ref<Ethereal::Texture2D> icon, ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, ImColor iconTint/* = IM_COL32_WHITE*/)
    {
        ...
        if (display_frame)
        {
            ...
            if (icon)
            {
                ...
                Ethereal::UI::ShiftCursorY(-frame_height + pad);
                Ethereal::UI::ShiftCursorX(arrowWidth);
                Ethereal::UI::Image(icon, { frame_height - pad * 2.0f, frame_height - pad * 2.0f }, ImVec2(0, 0), ImVec2(1, 1), iconTint /*selected ? ColorDark : tintFloat*/);
    ...
    bool TreeNodeWithIcon(Ethereal::Ref<Ethereal::Texture2D> icon, const void* ptr_id, ImGuiTreeNodeFlags flags, ImColor iconTint, const char* fmt, ...)
    {
        ...
        ImGuiContext& g = *GImGui;
        const char* label_end = g.TempBuffer.Data + ImFormatStringV(g.TempBuffer.Data, IM_ARRAYSIZE(g.TempBuffer.Data), fmt, args);
