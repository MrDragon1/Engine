...
void OpenGLDriverApi::SetViewport(uint32_t x, uint32_t y, uint32_t width, uint32_t height) {
    glViewport(x, y, width, height);
}

Ref<Texture> OpenGLDriverApi::CreateTexture(uint8_t levels, uint32_t width, uint32_t height,
...
Ref<BufferObject> OpenGLDriverApi::CreateBufferObject(uint32_t byteCount,
    ...
    glBufferData(handle->gl.binding, byteCount, nullptr,
                 GLUtils::ResolveBufferUsage(handle->usage));

    return handle;
}

Ref<VertexBuffer> OpenGLDriverApi::CreateVertexBuffer(AttributeArray attributeArray,
                                                      uint32_t vertexCount, uint8_t attributeCount,
                                                      uint8_t bufferCount) {
    Ref<GLVertexBuffer> vb =
        Ref<GLVertexBuffer>::Create(attributeArray, vertexCount, attributeCount, bufferCount);
    return vb;
}

Ref<IndexBuffer> OpenGLDriverApi::CreateIndexBuffer(ElementType elementType, uint32_t indexCount,
                                                    BufferUsage usage) {
...
Ref<RenderPrimitive> OpenGLDriverApi::CreateRenderPrimitive(Ref<VertexBuffer> vbh,
...
void OpenGLDriverApi::Draw(Ref<RenderPrimitive> rph, PipelineState pipeline) {
    ...
    for (auto& sg : mSamplerGroupBindings) {
        ...
        for (auto& entry : sg->entries) {
            ...
            if (entry.level == INVALID_UINT32 && entry.layer == INVALID_UINT32) {
                ...
                // The first parameter is the index of the image unit to be modified, no need to add
                // GL_TEXTURE0
                glBindImageTexture(offset + entry.binding, tex->gl.id,
                                   entry.level == INVALID_UINT32 ? 0 : entry.level,
                                   entry.layer == INVALID_UINT32 ? GL_FALSE : GL_TRUE,
                                   entry.layer == INVALID_UINT32 ? 0 : entry.layer, GL_READ_WRITE,
                                   GL_RGBA16F);
            }
        }
    ...
    glDrawElementsBaseVertex(GLenum(rp->type), (GLsizei)rp->count, rp->gl.getIndicesType(),
                             reinterpret_cast<const void*>(rp->offset), rp->minIndex);
...
void OpenGLDriverApi::BeginRenderPass(RenderTargetHandle rth, const RenderPassParams& params) {
    ...
    glClearColor(params.clearColor.x, params.clearColor.y, params.clearColor.z,
                 params.clearColor.w);
...
void OpenGLDriverApi::SetVertexBufferObject(Ref<VertexBuffer> vbh, uint32_t index,
...
void OpenGLDriverApi::UpdateBufferObject(Ref<BufferObject> handle, BufferDescriptor&& bd,
                                         uint32_t byteOffset) {
    ...
    if (byteOffset == 0 && bd.size == bo->byteCount) {
        ...
        glBufferData(bo->gl.binding, (GLsizeiptr)bd.size, bd.buffer,
                     GLUtils::ResolveBufferUsage(bo->usage));
    } else {
...
void OpenGLDriverApi::UpdateIndexBuffer(Ref<IndexBuffer> handle, BufferDescriptor&& bd,
                                        uint32_t byteOffset) {
...
void OpenGLDriverApi::SetTextureData(Ref<Texture> th, uint32_t levels, uint32_t xoffset,
    ...
    switch (t->gl.target) {
        ...
        case GL_TEXTURE_3D:
            glTexSubImage3D(t->gl.target, levels, xoffset, yoffset, zoffset, width, height, depth,
                            format, type, desc.buffer);
            break;
        case GL_TEXTURE_CUBE_MAP:
            ET_CORE_ASSERT(zoffset + depth <= 6)
            ET_CORE_ASSERT(width == height)
            const size_t faceSize = PixelBufferDescriptor::ComputeDataSize(
                desc.dataFormat, desc.dataType, desc.stride ? desc.stride : width, height,
                desc.alignment);
...
void OpenGLDriverApi::UpdateSamplerGroup(SamplerGroupHandle sgh, SamplerGroupDescriptor& desc) {
    ...
    for (size_t i = 0; i < desc.size(); ++i) {
        ...
        if (desc[i].texture) {
            ...
            glSamplerParameteri(entry.id, GL_TEXTURE_MIN_FILTER,
                                (GLint)GLUtils::ResolveSamplerMinFilter(params.filterMin));
            glSamplerParameteri(entry.id, GL_TEXTURE_MAG_FILTER,
                                (GLint)GLUtils::ResolveSamplerMagFilter(params.filterMag));
            glSamplerParameteri(entry.id, GL_TEXTURE_WRAP_S,
                                (GLint)GLUtils::ResolveSamplerWrapMode(params.wrapS));
            glSamplerParameteri(entry.id, GL_TEXTURE_WRAP_T,
                                (GLint)GLUtils::ResolveSamplerWrapMode(params.wrapT));
            glSamplerParameteri(entry.id, GL_TEXTURE_WRAP_R,
                                (GLint)GLUtils::ResolveSamplerWrapMode(params.wrapR));
            glSamplerParameteri(entry.id, GL_TEXTURE_COMPARE_MODE,
                                (GLint)GLUtils::ResolveSamplerCompareMode(params.compareMode));
            glSamplerParameteri(entry.id, GL_TEXTURE_COMPARE_FUNC,
                                (GLint)GLUtils::ResolveSamplerCompareFunc(params.compareFunc));
        }
    }
}

void OpenGLDriverApi::BindSamplerGroup(uint8_t binding, Ref<SamplerGroup> sgh) {
...
void OpenGLDriverApi::GetSubTexture(TextureHandle th, uint32_t layer, TextureHandle dsth) {
    ...
    ET_CORE_ASSERT(dst->width == t->width && dst->height == t->height,
                   "dst texture must has same size with src texture");
    glCopyImageSubData(t->gl.id, t->gl.target, 0, 0, 0, layer, dst->gl.id, GL_TEXTURE_2D, 0, 0, 0,
                       0, t->width, t->height, 1);
}

int OpenGLDriverApi::ReadPixel(RenderTargetHandle rth, uint32_t attachmentIndex, uint32_t xoffset,
                               uint32_t yoffset) {
    Ref<GLRenderTarget> rt = rth.As<GLRenderTarget>();
    glBindFramebuffer(GL_FRAMEBUFFER, rt->gl.id);
...
void OpenGLDriverApi::SetRenderTargetAttachment(RenderTargetHandle rth,
...
//---------------------------------------------------------------------
void OpenGLDriverApi::AllocateTexture(Ref<GLTexture> t, uint32_t width, uint32_t height,
                                      uint32_t depth) {
        ...
        if (bi != Attribute::BUFFER_UNUSED && vb->gl.buffers[bi] != 0) {
            ...
            if (attribute.flags & Attribute::FLAG_INTEGER_TARGET) {
                glVertexAttribIPointer(GLuint(i),
                                       (GLint)GLUtils::ResolveElementTypeCount(attribute.type),
                                       GLUtils::ResolveElementType(attribute.type),
                                       attribute.stride, (void*)uintptr_t(attribute.offset));
            } else {
                glVertexAttribPointer(
                    GLuint(i), (GLint)GLUtils::ResolveElementTypeCount(attribute.type),
                    GLUtils::ResolveElementType(attribute.type),
                    GLUtils::ResolveNormalize(attribute.flags & Attribute::FLAG_NORMALIZED),
                    attribute.stride, (void*)uintptr_t(attribute.offset));
            }
...
void OpenGLDriverApi::UpdateFrameBufferTexture(Ref<GLRenderTarget> rt, TargetBufferInfo const& info,
                                               GLenum attachment) {
    ...
    ET_CORE_ASSERT((rt->width <= valueForLevel(info.level, t->width) &&
                    rt->height <= valueForLevel(info.level, t->height)),
    ...
    switch (target) {
        case GL_TEXTURE_CUBE_MAP:
            ...
            glFramebufferTexture2D(GL_FRAMEBUFFER, attachment,
                                   GL_TEXTURE_CUBE_MAP_POSITIVE_X + info.layer, t->gl.id,
                                   info.level);
            break;
        case GL_TEXTURE_2D:
        ...
        case GL_TEXTURE_CUBE_MAP_ARRAY: {
            if (info.layer == INVALID_UINT32)
                glFramebufferTexture(GL_FRAMEBUFFER, attachment, t->gl.id,
                                     info.level);  // multi layer render
            else
                glFramebufferTextureLayer(GL_FRAMEBUFFER, attachment, t->gl.id, info.level,
                                          info.layer);
            break;
        }
        default:
