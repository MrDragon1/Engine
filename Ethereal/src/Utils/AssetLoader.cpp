...
namespace Ethereal
{
    ...
    //            MeshVertex vertex;
    //            Vector3 vector;  // we declare a placeholder vector since assimp uses its own vector class that doesn't directly convert to glm's
    //            vec3
    //                               // class so we transfer the data to this placeholder Vector3 first.
    //            // positions
    ...
    void TextureLoader::LoadPath(const std::string& path, Ref<TextureData>& textureData, bool flip) {
        ...
        if (data) {
            ...
            if (hdr) {
                textureData->m_format = ETHEREAL_PIXEL_FORMAT::R16G16B16A16_HDR;
            } else {
                if (channels == 4) {
                    textureData->m_format = ETHEREAL_PIXEL_FORMAT::R8G8B8A8_UNORM;
                } else if (channels == 3) {
                    textureData->m_format = ETHEREAL_PIXEL_FORMAT::R8G8B8_UNORM;
                } else if (channels == 1) {
                    textureData->m_format = ETHEREAL_PIXEL_FORMAT::RED;
                }
            }
